...
def _batch_ts(df: pd.DataFrame) -> pd.Timestamp:
    # einheitlicher Batchzeitpunkt für den Node-Output
    return pd.Timestamp.utcnow()

def _derive_customer_from_id(series: pd.Series) -> pd.Series:
    # Kundennummer aus der ID ableiten: erste 4 Ziffern
...
def build_silver_products(
    ...
    bronze_prices_1003: pd.DataFrame = None,   # keep if you still pass it
) -> pd.DataFrame:
    if bronze_products_all is None or bronze_products_all.empty:
        return pd.DataFrame(columns=[
            "id_product","number_product","product_name","product_group","price","moq",
            "_customer_id","_ingest_ts","_row_hash"
        ])
    ...
    df = bp.merge(
        mp, on=["number_product", "_customer_id"], how="left", validate="m:1"
    )

    # (Optional) if you enrich prices in Silver; if you enriched in Bronze, remove this block
    if bronze_prices_1003 is not None and not bronze_prices_1003.empty:
        ...
        if "price_from_prices" in df.columns:
            df["price"] = df["price"].where(df["price"].notna(), df["price_from_prices"])
    ...
    keep = ["id_product","number_product","product_name","product_group","price","moq",
            "_customer_id","_ingest_ts","_row_hash"]
    for c in keep:
...
def build_silver_stores(
    ...
    df = bs.merge(
        ms, on=["number_store", "_customer_id"], how="left", validate="m:1"
    )
    ...
    keep = ["id_store","number_store","store_name",
            "street","postal_code","city","country","state","store_address",
            "_customer_id","_ingest_ts","_row_hash"]
    for c in keep:
        if c not in df.columns:
...
def build_silver_sales_daily(
    ...
    if bronze_sales_all is None or bronze_sales_all.empty:
        s_agg = pd.DataFrame(columns=["_customer_id","number_store","number_product","target_date","sales_qty"])
    else:
        s = bronze_sales_all.copy()
        s["sales_qty"] = pd.to_numeric(s.get("sales_qty", 0.0), errors="coerce").fillna(0.0)
        s["target_date"] = pd.to_datetime(s["target_date"], errors="coerce").dt.normalize()
        s_agg = (s.groupby(["_customer_id","number_store","number_product","target_date"], dropna=False)["sales_qty"]
                  .sum().reset_index())

    # --- 2) aggregate deliveries ---
    if bronze_deliveries_all is None or bronze_deliveries_all.empty:
        d_pre = pd.DataFrame(columns=["_customer_id","number_store","number_product","target_date","delivery_qty"])
    else:
        d_pre = bronze_deliveries_all.copy()
        d_pre["delivery_qty"] = pd.to_numeric(d_pre.get("delivery_qty"), errors="raise")
        d_pre["target_date"]  = pd.to_datetime(d_pre["target_date"], errors="raise").dt.normalize()
        ...
        if map_delivery2sales_1001 is not None and not map_delivery2sales_1001.empty:
            ...
            d_pre["delivery_qty_adj"] = d_pre["delivery_qty"] * d_pre["factor"].fillna(1.0)
        else:
        ...
        d_agg = (
            d_pre.groupby(
                ...
                dropna=False
            )["delivery_qty_adj"].sum().reset_index()
              .rename(columns={"number_product_adj":"number_product","delivery_qty_adj":"delivery_qty"})
        )

    # --- 3) merge to fact on *raw keys* ---
    ...
    if fact.empty:
        ...
        cols = ["id_product","id_store","target_date","sales_qty","return_qty","delivery_qty","stockout",
                "_customer_id","_ingest_ts","_row_hash","number_product","number_store"]
        return pd.DataFrame(columns=cols)

    fact["sales_qty"] = pd.to_numeric(fact["sales_qty"], errors="coerce").fillna(0.0)
    fact["delivery_qty"] = pd.to_numeric(fact["delivery_qty"], errors="coerce").fillna(0.0)
    fact["return_qty"] = 0.0

    # --- 4) compute stockout on raw keys (root cause fix) ---
    fact = fact.sort_values(keys_raw)
    def _compute_stockout(g: pd.DataFrame) -> pd.DataFrame:
        # balance_t = max(0, balance_{t-1} + delivery + returns - sales)
        net = g["delivery_qty"].fillna(0) + g["return_qty"].fillna(0) - g["sales_qty"].fillna(0)
        rb = net.cumsum()
        rb = rb.where(rb > 0, 0.0)
        prev_rb = rb.shift(1).fillna(0.0)
        st = (prev_rb == 0.0) & (g["sales_qty"] > 0.0)
        if len(st) > 0:
            st.iloc[0] = False  # first day never stockout
        out = g.copy()
        out["stockout"] = st.astype(bool)
        return out

    fact = (fact.groupby(["_customer_id","number_store","number_product"], group_keys=False)
                 .apply(_compute_stockout))

    # --- 5) map IDs AFTER stockout ---
    ...
    fact = fact.merge(pmap, on=["_customer_id","number_product"], how="left", validate="m:1")
    fact = fact.merge(smap, on=["_customer_id","number_store"],  how="left", validate="m:1")

    # --- 6) lineage + final cols ---
    fact["_ingest_ts"] = pd.Timestamp.utcnow()
    fact["_row_hash"] = pd.util.hash_pandas_object(
        fact[["_customer_id","number_store","number_product","target_date"]].astype(str).fillna(""),
        index=False
    ).astype("uint64")

    cols = ["id_product","id_store","target_date",
            "sales_qty","return_qty","delivery_qty","stockout",
            "_customer_id","_ingest_ts","_row_hash",
            "number_product","number_store"]
    for c in cols:
        if c not in fact.columns:
            fact[c] = pd.NA

    return fact[cols].sort_values(["id_store","id_product","target_date"]).reset_index(drop=True)
