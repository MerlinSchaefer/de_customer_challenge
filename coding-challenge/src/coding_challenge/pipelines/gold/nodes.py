...
def build_dim_product(silver_products: pd.DataFrame) -> pd.DataFrame:
    """
    Return the latest product row per id_product as a dimension table.

    Args:
        silver_products (pd.DataFrame): Silver product records with _ingest_ts.

    Returns:
        pd.DataFrame: Columns: id_product, number_product, product_name, product_group, moq, price_current.
    """
    ...
    df = df.sort_values(["id_product", "_ingest_ts"]).drop_duplicates(
        subset=["id_product"], keep="last"
    )
    ...
    out = pd.DataFrame(
        {
            ...
            "product_group": (
                df.get("product_group").astype("string")
                if "product_group" in df
...
def build_dim_store(silver_stores: pd.DataFrame) -> pd.DataFrame:
    """
    Return the latest store row per id_store with split and full address.

    Args:
        silver_stores (pd.DataFrame): Silver store records with _ingest_ts.

    Returns:
        pd.DataFrame: Columns: id_store, number_store, store_name, street, postal_code, city, country, state, store_address.
    """
...
def _dense_calendar(df: pd.DataFrame) -> pd.DataFrame:
    """
    Expand rows to a dense daily calendar per (id_store, id_product) and fill missing values.

    Args:
        df (pd.DataFrame): Expect cols id_store, id_product, target_date, sales_qty, return_qty, delivery_qty, stockout, price.

    Returns:
        pd.DataFrame: Dense daily rows with numeric fills and forward/backward-filled price.
    """
...
def build_fact_daily_store_product(
    ...
    """
    Build the daily store×product fact table, optionally materializing pack-SKU deliveries.

    Args:
        silver_sales_daily (pd.DataFrame): Silver sales daily fact.
        dim_product (pd.DataFrame): Product dimension (used for prices).
        dim_store (pd.DataFrame): Store dimension (not used directly here).
        bronze_deliveries_all (pd.DataFrame): Bronze deliveries (optional).
        mapping_product_union (pd.DataFrame): Mapping from number_product to id_product.
        mapping_store_union (pd.DataFrame): Mapping from number_store to id_store.
        delivery_to_sales_map (list[dict], optional): Mapping rules for pack SKUs (delivery -> sales).

    Returns:
        pd.DataFrame: Columns id_product, id_store, target_date, sales_qty, return_qty, delivery_qty, stockout, price.
    """
    ...
    # Extra: Pack-SKU-Reihe(n) (z.B. 29) aus Bronze-Lieferungen materialisieren
    extras = []
    if (
        bronze_deliveries_all is not None
        and not bronze_deliveries_all.empty
        and delivery_to_sales_map
    ):
        ...
        if not m.empty:
            ...
            m["number_product_delivery"] = (
                m["number_product_delivery"].astype("string").str.strip()
            )
            m["number_product_sales"] = (
                m["number_product_sales"].astype("string").str.strip()
            )
            ...
            if not dlv.empty:
                ...
                pmap = (
                    mapping_product_union[
                        ["id_product", "number_product", "_customer_id"]
                    ]
                ...
                dlv = dlv.merge(
                    pmap,
                    left_on=["_customer_id", "number_product"],
                    right_on=["_customer_id", "number_product"],
                    how="left",
                    validate="m:1",
                )
                dlv = dlv.merge(
                    smap,
                    left_on=["_customer_id", "number_store"],
                    right_on=["_customer_id", "number_store"],
                    how="left",
                    validate="m:1",
                )
                ...
                g = (
                    dlv.groupby(
                        ["id_store", "id_product", "target_date"], dropna=False
                    )["delivery_qty"]
                    .sum()
                    .reset_index()
                )
                ...
                price_map = dim_product[
                    ["id_product", "price_current"]
                ].drop_duplicates()
    ...
    # sort
    fact_all = fact_all.sort_values(
        ["id_store", "id_product", "target_date"]
...
def build_view_features_ml_daily(fact: pd.DataFrame) -> pd.DataFrame:
    """
    Create ML features view with minimal columns.

    Args:
        fact (pd.DataFrame): Fact table containing sales and stockout info.

    Returns:
        pd.DataFrame: Columns: id_product, id_store, target_date, sales_qty, stockout.
    """
    if fact is None or fact.empty:
        return pd.DataFrame(
            columns=["id_product", "id_store", "target_date", "sales_qty", "stockout"]
        )

    df = fact[["id_product", "id_store", "target_date", "sales_qty", "stockout"]].copy()
    df["target_date"] = pd.to_datetime(df["target_date"], errors="raise").dt.date
    df["stockout"] = df["stockout"].astype(bool)
    return df.sort_values(["id_store", "id_product", "target_date"]).reset_index(
        drop=True
    )


def build_view_app_daily(
    fact: pd.DataFrame, dim_product: pd.DataFrame, dim_store: pd.DataFrame
) -> pd.DataFrame:
    """
    Create application view by joining fact with product and store dimensions.

    Args:
        fact (pd.DataFrame): Fact table.
        dim_product (pd.DataFrame): Product dimension.
        dim_store (pd.DataFrame): Store dimension.

    Returns:
        pd.DataFrame: Columns for app consumption including product/store metadata and prices.
    """
    if fact is None or fact.empty:
        return pd.DataFrame(
            columns=[
