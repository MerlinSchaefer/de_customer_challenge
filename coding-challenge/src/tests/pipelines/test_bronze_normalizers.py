...
def test_normalize_cosmos_sales_bronze_iso():
    ...
    ingestion_config = {
        "erps": {
            "cosmos": {
                "columns": {
                    "sales": {"date": "Datum", "store": "Kunde", "product": "Artikel", "qty": "VK-Menge"}
                }
            }
        }
    }
    out = normalize_cosmos_sales_bronze(raw, ingestion_config, customer_id="1001")
    assert list(out.columns) == ["target_date","number_store","number_product","sales_qty","_customer_id","_source_file"]
...
def test_normalize_galaxy_prices_bronze_daily_wrapper():
    ...
    ingestion_config = {
        "erps": {
            "galaxy": {
                "prices": {"wrapper": "Verkaufspreise", "product": "ArtikelNummer", "price": "ArtikelPreis"}
            }
        }
    }
    out = normalize_galaxy_prices_bronze_daily(raw, ingestion_config, customer_id="1003")
    assert list(out.columns) == ["target_date","number_product","price","_customer_id"]
