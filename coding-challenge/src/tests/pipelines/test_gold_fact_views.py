...
def test_gold_fact_dense_calendar_and_price_and_extras():
    ...
    dim_store = pd.DataFrame({
        ...
        "street":["Kosmonautengasse 1"],"postal_code":["13353"],"city":["Glitzerstadt"],
        "country":["Deutschland"],"state":["Bayern"],"store_address":["Kosmonautengasse 1 – 13353 – Glitzerstadt"],
    })
    ...
    delivery_to_sales_map = [
        {"_customer_id":"1001","number_product_delivery":"29","number_product_sales":"72","factor":12}
    ]

    fact = build_fact_daily_store_product(
        sales_daily, dim_product, dim_store,
        bronze_deliveries_all, mapping_product_union, mapping_store_union, delivery_to_sales_map
    )

    # 1) Dense calendar: 2025-08-10..2025-08-12 must be present
    dates = fact.query("id_store==100190001 & id_product==10010003")["target_date"].tolist()
    assert [str(d) for d in dates] == ["2025-08-10","2025-08-11","2025-08-12"]

    # 2) Pack-SKU rows exist (delivery-only):
    pack = fact.query("id_product==10010002 & id_store==100190001").sort_values("target_date")
    ...
    assert list(ml.columns) == ["id_product","id_store","target_date","sales_qty","stockout"]

    # 4) App view contains names and address
    app = build_view_app_daily(fact, dim_product, dim_store)
    assert {"product_name","number_product","store_name","store_address"} <= set(app.columns)
